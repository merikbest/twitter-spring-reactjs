...
describe("UserPage", () => {
    ...
    it("should User Profile Success Loaded", () => {
        ...
        expect(mockDispatchFn).nthCalledWith(3, {payload: {userId: "2", page: 0}, type: UserTweetsActionType.FETCH_TWEETS});
    ...
    it("should click tweet Tab and fetch user tweets", () => {
        ...
        expect(mockDispatchFn).nthCalledWith(5, {payload: {userId: "2", page: 0}, type: UserTweetsActionType.FETCH_TWEETS});
    });
    ...
    it("should click Tweets & replies Tab and fetch user tweets", () => {
        ...
        expect(wrapper.find(Tab).at(1).text().includes("Tweets & replies")).toBe(true);
        expect(mockDispatchFn).nthCalledWith(4, {type: UserTweetsActionType.RESET_TWEETS});
        expect(mockDispatchFn).nthCalledWith(5, {payload: {userId: "2", page: 0}, type: UserTweetsActionType.FETCH_RETWEETS_AND_REPLIES});
    });
    ...
    it("should click Media Tab and fetch user tweets", () => {
        ...
        expect(wrapper.find(Tab).at(2).text().includes("Media")).toBe(true);
        expect(mockDispatchFn).nthCalledWith(4, {type: UserTweetsActionType.RESET_TWEETS});
        expect(mockDispatchFn).nthCalledWith(5, {payload: {userId: "2", page: 0}, type: UserTweetsActionType.FETCH_MEDIA_TWEETS});
    });
    ...
    it("should click Likes Tab and fetch user tweets", () => {
        ...
        expect(wrapper.find(Tab).at(3).text().includes("Likes")).toBe(true);
        expect(mockDispatchFn).nthCalledWith(4, {type: UserTweetsActionType.RESET_TWEETS});
        expect(mockDispatchFn).nthCalledWith(5, {payload: {userId: "2", page: 0}, type: UserTweetsActionType.FETCH_LIKED_TWEETS});
    });
    ...
    it("should show single tweet count", () => {
        const mockCreatedState = createMockRootState();
        const wrapper = mountWithStore(<UserPage/>, {
            ...mockCreatedState,
            user: {
                ...mockCreatedState.user,
                data: changeSingleTweetsCount
            }
        });
        expect(wrapper.text().includes(`${mockUser.tweetCount} Tweet`)).toBe(true);
    });
    
    const changeSingleTweetsCount = (): AuthUserResponse => {
        return {...mockUser, tweetCount: 1, mediaTweetCount: 1, likeCount: 1};
    };
});
